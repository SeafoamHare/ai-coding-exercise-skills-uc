...
```java
@GetMapping
public ResponseEntity<List<PbiDto>> getPbis(
...
## DTO ID Naming Convention (CRITICAL)

**後端 DTO 的 ID 欄位必須使用 `[aggregate]Id` 命名，禁止使用泛用的 `id`。**

```java
// ✅ CORRECT — 使用 aggregateId
public record ProductDto(String productId, String name, ...) {}
public record SprintDto(String sprintId, String name, ...) {}
