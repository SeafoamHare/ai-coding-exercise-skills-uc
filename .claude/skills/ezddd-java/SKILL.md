...
| Pattern | 參考檔案 | 使用時機 |
|---------|---------|---------|
| Aggregate Root | `references/patterns/domain/aggregate.md` | 建立或修改 Aggregate（含 Rule 13 回傳 read-only） |
| Entity | `references/patterns/domain/entity.md` | 建立子 Entity（含 Rule 11 Read-only Entity） |
| **Read-only Entity** | `references/patterns/domain/entity.md` § Rule 11、`references/rules/domain-patterns.md` § Read-only Entities | Aggregate root 回傳 child entity 時，產生 `ReadOnly{Entity}`（JISE 2026 §2；對照範例 `Examle-2/`） |
