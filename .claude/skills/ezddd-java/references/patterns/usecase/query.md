...
- **Projection Interface** - Port layer (outbound port)
- **JPA Projection** - Adapter layer (database access)
- **DTO** - Data transfer object for query results
...
| Projection Interface | `src/main/java/{rootPackage}/{aggregate}/usecase/port/out/projection/{Name}Projection.java` |
| JPA Projection | `src/main/java/{rootPackage}/{aggregate}/adapter/out/database/springboot/projection/Jpa{Name}Projection.java` |
| DTO | `src/main/java/{rootPackage}/{aggregate}/usecase/port/{Name}Dto.java` |
...
**Returns a single DTO wrapped in CqrsOutput.**
...
public interface GetProductUseCase extends Query<GetProductUseCase.GetProductInput, GetProductUseCase.GetProductOutput> {
    ...
    class GetProductOutput extends CqrsOutput<GetProductOutput> {
        ...
        public GetProductOutput setProductDto(ProductDto dto) { this.productDto = dto; return this; }
    }
}
...
public class GetProductService implements GetProductUseCase {
    ...
    public GetProductOutput execute(GetProductInput input) {
        ...
        ProductDto dto = projection.findById(input.productId);
...
public interface GetProductsUseCase extends Query<GetProductsUseCase.GetProductsInput, GetProductsUseCase.GetProductsOutput> {
    ...
    class GetProductsOutput extends CqrsOutput<GetProductsOutput> {
        ...
        public GetProductsOutput setProducts(List<ProductDto> products) {
...
public class GetProductsService implements GetProductsUseCase {
    ...
    public GetProductsOutput execute(GetProductsInput input) {
        var projectionInput = new ProductsProjection.ProductsProjectionInput();
        projectionInput.userId = input.userId;
        projectionInput.sortBy = input.sortBy;

        List<ProductDto> products = projection.query(projectionInput);
...
public interface GetTasksByDateUseCase extends Query<GetTasksByDateUseCase.GetTasksByDateInput, GetTasksByDateUseCase.GetTasksByDateOutput> {
    ...
    class GetTasksByDateOutput extends CqrsOutput<GetTasksByDateOutput> {
        ...
        public GetTasksByDateOutput setTasks(List<TaskDto> tasks) { this.tasks = tasks; return this; }
    }
}
...
public interface GetProductUseCase extends Query<GetProductInput, ProductDto> { }  // Compilation error!
...
List<ProductDto> products = projection.query(input);

// For simple single-item lookup (custom method)
ProductDto product = projection.findById(productId);

// ❌ WRONG: Using execute() on Projection
List<ProductDto> products = projection.execute(input);  // WRONG method name!
```
...
@Override
public ProductDto execute(GetProductInput input) {
...
### Rule 5: Return DTO (wrapped in CqrsOutput), NOT Domain Entity

```java
// ✅ CORRECT: DTO wrapped in CqrsOutput Output class
public interface GetProductUseCase extends Query<..., GetProductUseCase.GetProductOutput> {
    class GetProductOutput extends CqrsOutput<GetProductOutput> {
        private ProductDto productDto;  // DTO inside CqrsOutput wrapper
    }
}

// ✅ CORRECT: Projection returns bare DTO (Projection is not constrained by CqrsOutput)
public interface ProductProjection extends Projection<..., ProductDto> {
    ProductDto findById(String productId);
}

// ❌ WRONG: Returning domain entity
public interface GetProductUseCase extends Query<..., Product> {  // Domain entity leak!
}

// ❌ WRONG: Bare DTO as Query output (violates CqrsOutput constraint)
public interface GetProductUseCase extends Query<..., ProductDto> {  // Compilation error!
}
```

**Rationale:** Domain entities should not leak outside the domain layer. Query Output must extend `CqrsOutput<T>` (CRTP). Projection interface is not constrained — it can return bare DTO.
...
public interface ProductsProjection extends Projection<ProductsProjection.Input, List<ProductDto>> {
...
public interface JpaProductsProjection extends ProductsProjection, JpaRepository<ProductData, String> {
    ...
    @Override
    default List<ProductDto> query(Input input) {
        Sort sort = Sort.by(
...
public record ProductDto(
    ...
    public static ProductDto from(ProductData data) {
...
public interface GetProductUseCase
    ...
    class GetProductOutput extends CqrsOutput<GetProductOutput> {
        ...
        public ProductDto getProductDto() { return productDto; }
        public GetProductOutput setProductDto(ProductDto dto) {
            this.productDto = dto;
            return this;
        }
    }
...
public interface GetProductUseCase extends Query<GetProductInput, ProductDto> {
    // Compilation error: ProductDto doesn't extend CqrsOutput
}

// ❌ WRONG: Using Optional as return type
public interface GetProductUseCase extends Query<GetProductInput, Optional<ProductDto>> {
...
GetProductOutput output = new GetProductOutput()
    .setProductDto(dto)        // returns GetProductOutput
...
public class PbiInMemoryRepositoryConfig {
    ...
    public PbisByProductProjection pbisByProductProjection(
        ...
        return input -> pbiDataStore.values().stream()
                ...
                .map(ProductBacklogItemMapper::toDto)
...
public record ${Name}Dto(
    ...
    public static ${Name}Dto from(${Name}Data data) {
        return new ${Name}Dto(
            data.get${Field1}(),
            data.get${Field2}()
        );
...
import ${rootPackage}.${aggregateLowerCase}.usecase.port.${Name}Dto;
...
public interface Jpa${Name}Projection extends ${Name}Projection, JpaRepository<${Name}Data, String> {
    ...
    default ${ReturnType} query(${Name}Input input) {
        ...
        return ${Name}Mapper.toDto(data);
    }

    // For single-item queries
    @Override
    default ${Name}Dto findById(String id) {
        return findById(id)
            .map(${Name}Dto::from)
            .orElse(null);
    }
...
public interface ${UseCase}UseCase extends Query<${UseCase}UseCase.${UseCase}Input, ${UseCase}UseCase.${UseCase}Output> {
    ...
    class ${UseCase}Output extends CqrsOutput<${UseCase}Output> {
        private ${Name}Dto ${nameCamelCase}Dto;  // or List<${Name}Dto> for list queries

        public ${Name}Dto get${Name}Dto() { return ${nameCamelCase}Dto; }
        public ${UseCase}Output set${Name}Dto(${Name}Dto dto) {
            this.${nameCamelCase}Dto = dto;
            return this;
        }
    }
}
...
public class ${UseCase}Service implements ${UseCase}UseCase {
    ...
    public ${UseCase}Output execute(${UseCase}Input input) {
        ...
        ${Name}Dto dto = projection.query(projectionInput);
...
public interface ProductsProjection extends Projection<ProductsProjection.ProductsInput, List<ProductDto>> {
